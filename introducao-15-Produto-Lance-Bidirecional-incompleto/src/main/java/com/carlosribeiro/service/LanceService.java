...
public class LanceService {
    ...
    private final LanceDAO lanceDAO = new LanceDaoImpl();// FabricaDeDaos...
    ...
    public Lance incluir(Lance lance) {
        if(lance.getProduto().getLances().isEmpty()) {
            if(lance.getValor() < lance.getProduto().getLanceMinimo()) throw new ValorDeLanceInvalidoException(
                    "")
        }
